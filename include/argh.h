...
public:
    ...
    // If the conversion was not possible, the stream will enter the
    // fail state, and operator bool will return false.
    template <typename T>
...
public:
    ...
    void
    add_params(std::initializer_list<char const *const> init_list);
    ...
    // flag (boolean) accessors: return true if the flag appeared,
    // otherwise false.
    bool operator[](std::string const &name) const;

    // multiple flag (boolean) accessors: return true if at least one
    // of the flag appeared, otherwise false.
    bool operator[](
        std::initializer_list<char const *const> init_list) const;

    // returns positional arg string by order. Like argv[] but without
    // the options
    std::string const &operator[](size_t ind) const;

    // returns a std::istream that can be used to convert a positional
    // arg to a typed value.
    string_stream operator()(size_t ind) const;

    // same as above, but with a default value in case the arg is
    // missing (index out of range).
    template <typename T>
    string_stream operator()(size_t ind, T &&def_val) const;

    // parameter accessors, give a name get an std::istream that can
    // be used to convert to a typed value. call .str() on result to
    // get as string
    string_stream operator()(std::string const &name) const;

    // accessor for a parameter with multiple names, give a list of
    // names, get an std::istream that can be used to convert to a
    // typed value. call .str() on result to get as string returns the
    // first value in the list to be found.
    string_stream operator()(
        std::initializer_list<char const *const> init_list) const;

    // same as above, but with a default value in case the param was
    // missing. Non-string def_val types must have an operator<<()
    // (output stream operator) If T only has an input stream
    // operator, pass the string version of the type as in "3" instead
    // of 3.
    template <typename T>
    string_stream operator()(std::string const &name,
                             T &&def_val) const;

    // same as above but for a list of names. returns the first value
    // to be found.
    template <typename T>
    string_stream
...
parser::parse(int argc,
    ...
    for (auto i = 0u; i < args_.size(); ++i) {
            ...
            !is_param(name))  // unregistered
        {
            ...
            if (!name.empty() &&
                is_param(std::string(
                    1ul, name.back())))  // last char is param
            {
            ...
            if (!keep_param.empty()) {
                ...
                continue;  // do not consider other options for this
                           // arg
            }
        }

        // any potential option will get as its value the next arg,
        // unless that arg is an option too in that case it will be
        // determined a flag.
        ...
        // if 'name' is a pre-registered option, then the next arg
        // cannot be a free parameter to it is skipped otherwise we
        // have 2 modes: PREFER_FLAG_FOR_UNREG_OPTION: a
        // non-registered 'name' is determined a flag.
        //                               The following value (the next
        //                               arg) will be a free
        //                               parameter.
        //
        // PREFER_PARAM_FOR_UNREG_OPTION: a non-registered 'name' is
        // determined a parameter, the next arg
        //                                will be the value of that
        //                                option.
...
parser::operator()(std::string const &name) const {
...
// same as above but for a list of names. returns the first value to
// be found.
template <typename T>
string_stream
