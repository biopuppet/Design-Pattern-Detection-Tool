...
int SubPatternDetector::detect_sp_instances(GCDR &system, GCDR &sp) {
    ...
    case 3: {
        ...
        edge_descriptor_t sp_es[] = {
            edge(0, 1, sp).first, edge(0, 2, sp).first, edge(1, 0, sp).first,
            edge(1, 2, sp).first, edge(2, 0, sp).first, edge(2, 1, sp).first};
        for (auto vd1 : cvs[0]) {
            for (auto vd2 : cvs[1]) {
                for (auto vd3 : cvs[2]) {
